...
def generate_cult_autogen_files(config: str, local_config: str, base_mod_path: str):
    ...
    with open(str(base_mod_path / f"common/inline_scripts/cui/cult/tec_cult_declarations.txt")) as f:
        ...
        for line in f:
            if should_look_for_modifier_end:
                ...
                if matches and matches.group(1):
                    if cult:
                        cults.append([cult, icon, modifiers])
    ...
    with open(base_mod_path / "common/static_modifiers/zz_evolved_autogenerated_cult_static_modifiers.txt", 'w') as f:
        for cult, icon, modifiers in cults:
            if modifiers:
                f.write(textwrap.dedent(f"""\
    ...
    with Writer(base_mod_path / "localisation/english/zz_evolved_autogenerated_cult_l_english.yml") as writer:
        ...
        for cult, _, _ in cults:
            writer.write_localization(f"tec_has_{cult}", f"$tec_has_empire_cult$ §Y$tec_cult_{cult}$§!")
            writer.write_localization(f"tec_activate_cult_{cult}", f"$tec_activate_cult_effect$ §Y$tec_cult_{cult}$§!")
            writer.write_localization(f"tec_deactivate_cult_{cult}", f"$tec_deactivate_cult_effect$ §Y$tec_cult_{cult}$§!")
            writer.write_localization(f"tec_has_last_activated_{cult}", f"$tec_has_empire_cult$ §Y$tec_cult_{cult}$§!")
            writer.write_localization(f"tec_has_last_deactivated_{cult}", f"$tec_has_empire_cult$ §Y$tec_cult_{cult}$§!")
