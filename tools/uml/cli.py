...
def create_compat_inlines(config: str, local_config: str, base_mod_path: str):
    ...
    with open(base_mod_path / "common/inline_scripts/mod_support/inline_evolved_trigger_placeholders.txt", 'w') as f:
        f.write("# mod_support/inline_evolved_trigger_placeholders\n")
    ...
    with open(base_mod_path / "common/inline_scripts/mod_support/inline_evolved_effect_placeholders.txt", 'w') as f:
        f.write("# mod_support/inline_evolved_effect_placeholders\n")
        f.write("# This file is autogenerated\n")
    ...
    with open(base_mod_path / "common/inline_scripts/mod_support/inline_evolved_inlines_include.txt", 'w') as f:
        f.write("# mod_support/inline_evolved_inlines_include\n")
        f.write("# This file is autogenerated\n")

        for s in suffixes:
            f.write(textwrap.dedent(f"""\
                inline_script = {{
                    script = evolved_support/$include_script$_{s}
                    $parameters$
                }}
            """))
    ...
    with open(base_mod_path / "common/scripted_triggers/!!_evolved_addon_triggers.txt", 'w') as f:
        ...
        f.write(textwrap.dedent(f"""\
            tec_trigger_mod_support = {{
                optimize_memory
                OR = {{
                    {new_line.join(f"                    $trigger$_{s} = yes" for s in suffixes).strip()}
                }}
            }}
        """))
    ...
    with open(base_mod_path / "common/scripted_effects/!!_evolved_addon_effects.txt", 'w') as f:
        ...
        f.write(textwrap.dedent(f"""\
            tec_effect_mod_support = {{
                optimize_memory
                {new_line.join(f"                $effect$_{s} = [[parameters]{{ $parameters$ }}][[!parameters] yes]" for s in suffixes).strip()}
            }}
        """))
        for s in scripted_effects:
            f.write(textwrap.dedent(f"""\
                inline_script = {{
                    script = mod_support/inline_evolved_effect_placeholders
                    effect = {s}
                }}
            """))

    with open(base_mod_path / "common/scripted_effects/!!_evolved_autogenerated_effects.txt", 'w') as f:
        f.write("# This file is autogenerated\n")
        f.write(textwrap.dedent(f"""\
            tec_cache_country_councilor_triggers_autogen = {{
                optimize_memory
                
                {new_line.join(f"                tec_clean_councilor_cache = {{ type = {s} }}" for s in councilor_checks).strip()}
                
                every_owned_leader = {{
                    limit = {{
                        is_councilor = yes
                    }}
                    {new_line.join(f"                    tec_cache_councilor = {{ type = {s} }}" for s in councilor_checks).strip()}
                }}
            }}
        """))
